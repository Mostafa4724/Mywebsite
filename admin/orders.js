...
(function () {
  ...
  function buyerName(order) {
    ...
    return name || order.customer_email || "Anonymous";
  }
  ...
  function render() {
    ...
    const filtered = allOrders.filter((order) => {
      const rowText =
        ("#" + order.id + " " + buyerName(order) + " " + itemsLabel(order)).toLowerCase();
      const matchSearch = !search || rowText.includes(search);
      const matchStatus = status === "all" || (order.status || "").toLowerCase() === status;

      let matchDate = true;
      if (date && date !== "all" && order.created_at) {
    ...
    if (filtered.length === 0) {
      ...
      ordersTableBody.innerHTML = filtered
        .map((order) => {
          const statusLabel = STATUS_LABELS[(order.status || "").toLowerCase()] || order.status || "Pending";
          const cls = statusClass(order.status);
          return (
            '<tr data-status="' +
            escapeHtml((order.status || "pending").toLowerCase()) +
            '">' +
            "<td><strong>#" +
            order.id +
            "</strong></td>" +
            "<td>" +
            escapeHtml(buyerName(order)) +
            "</td>" +
            "<td>" +
            escapeHtml(itemsLabel(order)) +
            "</td>" +
            "<td>" +
            ...
            "</span></td>" +
            '<td class="table-actions"><a class="btn-text" href="view-order.html?id=' +
            order.id +
            '">View</a></td>' +
            "</tr>"
          );
    ...
    if (pageInfo) {
      pageInfo.textContent = "Showing 1-" + filtered.length + " of " + allOrders.length + " orders";
    }
    if (badge) badge.textContent = String(allOrders.length);
  }
