(function () {
  ...
  function initUI() {
    ...
    if (elms.menuToggle && elms.sidebar) {
      ...
      document.addEventListener("click", function (e) {
        if (
          ...
          elms.sidebar.classList.contains("open") &&
          !elms.sidebar.contains(e.target) &&
          !elms.menuToggle.contains(e.target)
        ) {
    ...
    if (elms.refundBtn) {
      elms.refundBtn.addEventListener("click", function () {
        elms.refundModal.classList.add("show");
      });
    }

    if (elms.refundModalCancel) {
      elms.refundModalCancel.addEventListener("click", function () {
    ...
    if (elms.refundModalConfirm) {
      elms.refundModalConfirm.addEventListener("click", function () {
        ...
        showToast(
          "Refund of $" +
            (orderData ? Number(orderData.total).toFixed(2) : "0.00") +
            " processed",
          "success"
        );
      });
    ...
    if (elms.printBtn) {
      elms.printBtn.addEventListener("click", function () {
        window.print();
      });
    }
  ...
  function renderTimeline() {
    ...
    var steps =
      elms.orderTimeline.querySelectorAll(".vo-tl-step");

    var lines =
      elms.orderTimeline.querySelectorAll(".vo-tl-line");

    var status =
      (orderData.status || "pending").toLowerCase();
  ...
  function renderStatusControl() {
    ...
    if (
      ...
      if (elms.lockedNotice) {
        elms.lockedNotice.style.display = "flex";
      }
    ...
    if (elms.lockedNotice) {
      elms.lockedNotice.style.display = "none";
    }
